...
class motion_op(nn.Module):
    ...
    def __init__(self,
        ...
        self.kgrid = nn.Parameter(kgrid.type(dtypes.real_dtype), requires_grad=False)
        self.rgrid = nn.Parameter(rgrid.type(dtypes.real_dtype), requires_grad=False)
        # self.rgrid = rgrid.type(dtypes.real_dtype)
        # self.kgrid = kgrid.type(dtypes.real_dtype)
        self.U = None
        self.V = None
    ...
    def _build_U(self,
        ...
        """
        return torch.exp(-1j * einsum(self.kgrid, translations.type(dtypes.real_dtype), 
                                      '... out, N out -> N ...'))
    ...
    def _build_V(self,
        ...
        roations_radians = torch.deg2rad(rotations).type(dtypes.real_dtype)
