...
def temporal_segmentation(st_imperf: spatio_temporal,
    ...
    im_size = st_imperf.im_size
    spatial_funcs = torch.zeros(L, *im_size, device=st_imperf.torch_dev, dtype=dtypes.complex_dtype)
    ...
    temporal_funcs = torch.zeros(L, *st_imperf.trj_size, device=st_imperf.torch_dev, dtype=dtypes.complex_dtype)
    if 'zero' in interp_type:
        ...
        AHA = torch.zeros((L, L), device=st_imperf.torch_dev, dtype=dtypes.complex_dtype)
        for i in range(L):
            for j in range(L):
                AHA[i, j] = (spatial_funcs[i].conj() * spatial_funcs[j]).mean()

        # Next compute AHy, which is essentially int_r W(r, t) * b_l(r).conj() dr
        AHy = torch.zeros((L, *st_imperf.trj_size), device=st_imperf.torch_dev, dtype=dtypes.complex_dtype)
