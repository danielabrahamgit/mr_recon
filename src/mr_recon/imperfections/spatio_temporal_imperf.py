...
class spatio_temporal:
    ...
    def test_matrix_prods(self,
        ...
        # Gen Inputs
        inp_spatial = torch.randn(self.im_size, device=self.torch_dev, dtype=dtypes.complex_dtype)
        inp_spatial_flt = inp_spatial.flatten()
        inp_temporal = torch.randn(self.trj_size, device=self.torch_dev, dtype=dtypes.complex_dtype)
...
class B0(spatio_temporal):
    ...
    def __init__(self, 
        ...
        if coil_maps is None:
            coil_maps = torch.ones((1, *im_size), device=b0_map.device, dtype=dtypes.complex_dtype)
        ...
        self.toeplitz_kerns = torch.zeros((C, C, num_readout * 2), device=b0_map.device, dtype=dtypes.complex_dtype)
        for c1 in range(C):
    ...
    def forward_matrix_prod(self,
        ...
        temporal_output = torch.zeros((self.mps.shape[0], *self.trj_size), device=self.b0_map.device, dtype=dtypes.complex_dtype)
        tup = (slice(None),) + (None,) * self.readout_dim + (slice(None),) + (None,) * (len(self.trj_size) - self.readout_dim - 1)
        # temporal_output = torch.zeros(self.trj_size, device=self.b0_map.device, dtype=dtypes.complex_dtype)
        # tup = (None,) * self.readout_dim + (slice(None),) + (None,) * (len(self.trj_size) - self.readout_dim - 1)
