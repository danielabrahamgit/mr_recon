...
def _get_interpolate(kernel):
    ...
    @nb.jit(nopython=True, cache=True)  # pragma: no cover
    def _interpolate4(output, input, coord, width, param):
        # input shape: (batch_size, n4, n3, n2, n1)
        batch_size, n4, n3, n2, n1 = input.shape
        npts = coord.shape[0]
        for i in range(npts):
            # Coordinates: note the reverse order relative to the input dimensions.
            kx = coord[i, -1]
            ky = coord[i, -2]
            kz = coord[i, -3]
            kw = coord[i, -4]
            ...
            x0 = np.ceil(kx - width[-1] / 2)
            x1 = np.floor(kx + width[-1] / 2)
            y0 = np.ceil(ky - width[-2] / 2)
            y1 = np.floor(ky + width[-2] / 2)
            z0 = np.ceil(kz - width[-3] / 2)
            z1 = np.floor(kz + width[-3] / 2)
            w0 = np.ceil(kw - width[-4] / 2)
            w1 = np.floor(kw + width[-4] / 2)
            for w in range(w0, w1 + 1):
                ww = kernel((w - kw) / (width[-4] / 2), param[-4])
                for z in range(z0, z1 + 1):
                    wz = ww * kernel((z - kz) / (width[-3] / 2), param[-3])
                    for y in range(y0, y1 + 1):
                        wy = wz * kernel((y - ky) / (width[-2] / 2), param[-2])
                        for x in range(x0, x1 + 1):
                            w_val = wy * kernel((x - kx) / (width[-1] / 2), param[-1])
                            for b in range(batch_size):
                                # Use modulo for circular boundary handling.
                                output[b, i] += w_val * input[b, w % n4, z % n3, y % n2, x % n1]
        return output

    @nb.jit(nopython=True, cache=True)  # pragma: no cover
    def _interpolate5(output, input, coord, width, param):
        # input shape: (batch_size, n5, n4, n3, n2, n1)
        batch_size, n5, n4, n3, n2, n1 = input.shape
        npts = coord.shape[0]
        for i in range(npts):
            # Extract coordinates: last coordinate is the x dimension.
            kx = coord[i, -1]
            ky = coord[i, -2]
            kz = coord[i, -3]
            kw = coord[i, -4]
            ...
            w0 = np.ceil(kw - width[-4] / 2)
            w1 = np.floor(kw + width[-4] / 2)
            v0 = np.ceil(kv - width[-5] / 2)
            v1 = np.floor(kv + width[-5] / 2)
            for v in range(v0, v1 + 1):
                vv = kernel((v - kv) / (width[-5] / 2), param[-5])
                for w in range(w0, w1 + 1):
                    ww = vv * kernel((w - kw) / (width[-4] / 2), param[-4])
                    for z in range(z0, z1 + 1):
                        wz = ww * kernel((z - kz) / (width[-3] / 2), param[-3])
                        for y in range(y0, y1 + 1):
                            wy = wz * kernel((y - ky) / (width[-2] / 2), param[-2])
                            for x in range(x0, x1 + 1):
                                w_val = wy * kernel((x - kx) / (width[-1] / 2), param[-1])
                                for b in range(batch_size):
                                    output[b, i] += w_val * input[b, v % n5, w % n4, z % n3, y % n2, x % n1]
        return output
    
    return _interpolate1, _interpolate2, _interpolate3, _interpolate4, _interpolate5
...
def _get_gridding(kernel):
    ...
    @nb.jit(nopython=True, cache=True)  # pragma: no cover
    def _gridding4(output, input, coord, width, param):
        # output shape: (batch_size, n4, n3, n2, n1)
        batch_size, n4, n3, n2, n1 = output.shape
        npts = coord.shape[0]
        for i in range(npts):
            kx = coord[i, -1]
            ky = coord[i, -2]
                ...
                for z in range(z0, z1 + 1):
                    ...
                    for y in range(y0, y1 + 1):
                        ...
                        for x in range(x0, x1 + 1):
                            w_val = wy * kernel((x - kx) / (width[-1] / 2), param[-1])
                            for b in range(batch_size):
                                output[b, w % n4, z % n3, y % n2, x % n1] += w_val * input[b, i]
        return output

    @nb.jit(nopython=True, cache=True)  # pragma: no cover
    def _gridding5(output, input, coord, width, param):
        # output shape: (batch_size, n5, n4, n3, n2, n1)
        batch_size, n5, n4, n3, n2, n1 = output.shape
        npts = coord.shape[0]
        for i in range(npts):
            kx = coord[i, -1]
            ky = coord[i, -2]
                    ...
                    for z in range(z0, z1 + 1):
                        ...
                        for y in range(y0, y1 + 1):
                            ...
                            for x in range(x0, x1 + 1):
                                w_val = wy * kernel((x - kx) / (width[-1] / 2), param[-1])
                                for b in range(batch_size):
                                    output[b, v % n5, w % n4, z % n3, y % n2, x % n1] += w_val * input[b, i]
        return output

    return _gridding1, _gridding2, _gridding3, _gridding4, _gridding5
...
if config.cupy_enabled:  # pragma: no cover
    ...
    def _get_interpolate_cuda(kernel):
        ...
        _interpolate4_cuda = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 4;
            const int batch_size = input.shape()[0];
            const int nw = input.shape()[1];
            const int nz = input.shape()[2];
            const int ny = input.shape()[3];
            const int nx = input.shape()[4];

            const int coordw_idx[] = {i, 0};
            const S kw = coord[coordw_idx];
            const int coordz_idx[] = {i, 1};
            const S kz = coord[coordz_idx];
            const int coordy_idx[] = {i, 2};
            const S ky = coord[coordy_idx];
            const int coordx_idx[] = {i, 3};
            const S kx = coord[coordx_idx];

            const int x0 = ceil(kx - width[ndim - 1] / 2.0);
            const int y0 = ceil(ky - width[ndim - 2] / 2.0);
            const int z0 = ceil(kz - width[ndim - 3] / 2.0);
            const int w0 = ceil(kw - width[ndim - 4] / 2.0);

            const int x1 = floor(kx + width[ndim - 1] / 2.0);
            const int y1 = floor(ky + width[ndim - 2] / 2.0);
            const int z1 = floor(kz + width[ndim - 3] / 2.0);
            const int w1 = floor(kw + width[ndim - 4] / 2.0);

            for (int w = w0; w < w1 + 1; w++) {
                const S ww = kernel(
                    ((S) w - kw) / (width[ndim - 4] / 2.0),
                    param[ndim - 4]);
                for (int z = z0; z < z1 + 1; z++) {
                    const S wz = ww * kernel(
                        ((S) z - kz) / (width[ndim - 3] / 2.0),
                        param[ndim - 3]);
                    for (int y = y0; y < y1 + 1; y++) {
                        const S wy = wz * kernel(
                            ...
                            param[ndim - 2]);
                        for (int x = x0; x < x1 + 1; x++) {
                            const S w_val = wy * kernel(
                                ((S) x - kx) / (width[ndim - 1] / 2.0),
                                param[ndim - 1]);
                            for (int b = 0; b < batch_size; b++) {
                                const int input_idx[] = {b, mod(w, nw), mod(z, nz), mod(y, ny), mod(x, nx)};
                                const T v = (T) w_val * input[input_idx];
                                const int output_idx[] = {b, i};
                                output[output_idx] += v;
                            }
                        }
        ...
        _interpolate5_cuda = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 5;
            const int batch_size = input.shape()[0];
            const int nv = input.shape()[1];
            const int nw = input.shape()[2];
            const int nz = input.shape()[3];
            const int ny = input.shape()[4];
            const int nx = input.shape()[5];

            const int coordv_idx[] = {i, 0};
            const S kv = coord[coordv_idx];
            const int coordw_idx[] = {i, 1};
            const S kw = coord[coordw_idx];
            const int coordz_idx[] = {i, 2};
            const S kz = coord[coordz_idx];
            const int coordy_idx[] = {i, 3};
            const S ky = coord[coordy_idx];
            const int coordx_idx[] = {i, 4};
            const S kx = coord[coordx_idx];
            ...
            const int z0 = ceil(kz - width[ndim - 3] / 2.0);
            const int w0 = ceil(kw - width[ndim - 4] / 2.0);
            const int v0 = ceil(kv - width[ndim - 5] / 2.0);

            const int x1 = floor(kx + width[ndim - 1] / 2.0);
            const int y1 = floor(ky + width[ndim - 2] / 2.0);
            const int z1 = floor(kz + width[ndim - 3] / 2.0);
            const int w1 = floor(kw + width[ndim - 4] / 2.0);
            const int v1 = floor(kv + width[ndim - 5] / 2.0);

            for (int v = v0; v < v1 + 1; v++) {
                const S vv = kernel(
                    ((S) v - kv) / (width[ndim - 5] / 2.0),
                    param[ndim - 5]);
                for (int w = w0; w < w1 + 1; w++) {
                    const S ww = vv * kernel(
                        ((S) w - kw) / (width[ndim - 4] / 2.0),
                        param[ndim - 4]);
                    for (int z = z0; z < z1 + 1; z++) {
                        const S wz = ww * kernel(
                        ...
                        for (int y = y0; y < y1 + 1; y++) {
                            ...
                            for (int x = x0; x < x1 + 1; x++) {
                                const S w_val = wy * kernel(
                                    ...
                                    param[ndim - 1]);
                                for (int b = 0; b < batch_size; b++) {
                                    const int input_idx[] = {b, mod(v, nv), mod(w, nw), mod(z, nz), mod(y, ny), mod(x, nx)};
                                    const T v = (T) w_val * input[input_idx];
                                    const int output_idx[] = {b, i};
                                    output[output_idx] += v;
                                }
        ...
        return _interpolate1_cuda, _interpolate2_cuda, _interpolate3_cuda, _interpolate4_cuda, _interpolate5_cuda
    ...
    def _get_gridding_cuda(kernel):
        ...
        _gridding4_cuda = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 4;
            const int batch_size = output.shape()[0];
            // Output grid dimensions: assume shape = [batch_size, n4, n3, n2, n1]
            const int n4 = output.shape()[1];
            const int n3 = output.shape()[2];
            const int n2 = output.shape()[3];
            const int n1 = output.shape()[4];
            ...
            // Compute lower and upper bounds in each dimension.
            const int w0 = ceil(kw - width[ndim - 4] / 2.0);
            const int z0 = ceil(kz - width[ndim - 3] / 2.0);
            const int y0 = ceil(ky - width[ndim - 2] / 2.0);
            const int x0 = ceil(kx - width[ndim - 1] / 2.0);
            ...
            // Loop over the grid neighborhood.
            for (int w = w0; w < w1 + 1; w++) {
                const S ww = kernel(((S) w - kw) / (width[ndim - 4] / 2.0), param[ndim - 4]);
                for (int z = z0; z < z1 + 1; z++) {
                    const S wz = ww * kernel(((S) z - kz) / (width[ndim - 3] / 2.0), param[ndim - 3]);
                    for (int y = y0; y < y1 + 1; y++) {
                        const S wy = wz * kernel(((S) y - ky) / (width[ndim - 2] / 2.0), param[ndim - 2]);
                        for (int x = x0; x < x1 + 1; x++) {
                            const S w_val = wy * kernel(((S) x - kx) / (width[ndim - 1] / 2.0), param[ndim - 1]);
                            for (int b = 0; b < batch_size; b++) {
                                // input is indexed by (b, i) where i is the point index.
                                const int input_idx[] = {b, i};
                                const T v = (T) w_val * input[input_idx];
                                // output grid index: [b, mod(w,n4), mod(z,n3), mod(y,n2), mod(x,n1)]
                                const int output_idx[] = {b, mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)};
                                atomicAdd(&output[output_idx], v);
                            }
                        }
                    }
        ...
        _gridding5_cuda = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 5;
            const int batch_size = output.shape()[0];
            // Output grid dimensions: assume shape = [batch_size, n5, n4, n3, n2, n1]
            const int n5 = output.shape()[1];
            const int n4 = output.shape()[2];
            const int n3 = output.shape()[3];
            const int n2 = output.shape()[4];
            const int n1 = output.shape()[5];
            ...
            // Loop over the grid neighborhood.
            for (int v = v0; v < v1 + 1; v++) {
                const S vv = kernel(((S) v - kv) / (width[ndim - 5] / 2.0), param[ndim - 5]);
                for (int w = w0; w < w1 + 1; w++) {
                    const S ww = vv * kernel(((S) w - kw) / (width[ndim - 4] / 2.0), param[ndim - 4]);
                    for (int z = z0; z < z1 + 1; z++) {
                        const S wz = ww * kernel(((S) z - kz) / (width[ndim - 3] / 2.0), param[ndim - 3]);
                        for (int y = y0; y < y1 + 1; y++) {
                            const S wy = wz * kernel(((S) y - ky) / (width[ndim - 2] / 2.0), param[ndim - 2]);
                            for (int x = x0; x < x1 + 1; x++) {
                                ...
                                for (int b = 0; b < batch_size; b++) {
                                    const int input_idx[] = {b, i};
                                    const T v_val = (T) w_val * input[input_idx];
                                    // Output grid index: [b, mod(v,n5), mod(w,n4), mod(z,n3), mod(y,n2), mod(x,n1)]
                                    const int output_idx[] = {b, mod(v, n5), mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)};
                                    atomicAdd(&output[output_idx], v_val);
                                }
                            }
                        }
                    }
        ...
        return _gridding1_cuda, _gridding2_cuda, _gridding3_cuda, _gridding4_cuda, _gridding5_cuda
    ...
    def _get_gridding_cuda_complex(kernel):
        ...
        _gridding4_cuda_complex = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 4;
            ...
            // Coordinate extraction: assume coord[i,0]=w, coord[i,1]=z, coord[i,2]=y, coord[i,3]=x.
            const int coordw_idx[] = {i, 0};
            const S kw = coord[coordw_idx];
            const int coordz_idx[] = {i, 1};
            const S kz = coord[coordz_idx];
            ...
            // Compute lower and upper bounds for each dimension.
            const int w0 = ceil(kw - width[ndim - 4] / 2.0);
            const int z0 = ceil(kz - width[ndim - 3] / 2.0);
            const int y0 = ceil(ky - width[ndim - 2] / 2.0);
            const int x0 = ceil(kx - width[ndim - 1] / 2.0);
                    ...
                    for (int y = y0; y < y1 + 1; y++) {
                        ...
                        for (int x = x0; x < x1 + 1; x++) {
                            ...
                            for (int b = 0; b < batch_size; b++) {
                                ...
                                // Output grid index: [b, mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)]
                                const int output_idx[] = {b, mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)};
                                // For complex values, add to real and imaginary parts separately.
            ...
            """,
            name="gridding4_complex",
            preamble=kernel + mod_cuda,
            reduce_dims=False,
        )

        _gridding5_cuda_complex = cp.ElementwiseKernel(
            "raw T input, raw S coord, raw S width, raw S param",
            "raw T output",
            """
            const int ndim = 5;
            ...
            // Coordinate extraction: assume coord[i,0]=v, coord[i,1]=w, coord[i,2]=z, coord[i,3]=y, coord[i,4]=x.
            const int coordv_idx[] = {i, 0};
            const S kv = coord[coordv_idx];
            const int coordw_idx[] = {i, 1};
            const S kw = coord[coordw_idx];
            ...
            for (int v = v0; v < v1 + 1; v++) {
                const S wv = kernel(((S) v - kv) / (width[ndim - 5] / 2.0), param[ndim - 5]);
                for (int w = w0; w < w1 + 1; w++) {
                    const S ww = wv * kernel(((S) w - kw) / (width[ndim - 4] / 2.0), param[ndim - 4]);
                    for (int z = z0; z < z1 + 1; z++) {
                        const S wz = ww * kernel(((S) z - kz) / (width[ndim - 3] / 2.0), param[ndim - 3]);
                        for (int y = y0; y < y1 + 1; y++) {
                            const S wy = wz * kernel(((S) y - ky) / (width[ndim - 2] / 2.0), param[ndim - 2]);
                            for (int x = x0; x < x1 + 1; x++) {
                                ...
                                for (int b = 0; b < batch_size; b++) {
                                    ...
                                    // Output grid index: [b, mod(v, n5), mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)]
                                    const int output_idx[] = {b, mod(v, n5), mod(w, n4), mod(z, n3), mod(y, n2), mod(x, n1)};
                                    atomicAdd(reinterpret_cast<T::value_type*>(&(output[output_idx])), v_val.real());
                                    atomicAdd(reinterpret_cast<T::value_type*>(&(output[output_idx])) + 1, v_val.imag());
                                }
                            }
                        }
                    }
        ...
        return (
            ...
            _gridding4_cuda_complex,
            _gridding5_cuda_complex,
        )
